...
int main() {
  ...
  */
  VALUE_CALL_OR_DIE(BF_Init(LRU));
  VALUE_CALL_OR_DIE(BF_CreateFile("block_example.db"))
  VALUE_CALL_OR_DIE(BF_OpenFile("block_example.db", &fd1));

  void* data;
  for (int i = 0; i < 10; ++i) {
    VALUE_CALL_OR_DIE(BF_AllocateBlock(fd1, block));  // Δημιουργία καινούριου block
  ...
  VALUE_CALL_OR_DIE(BF_CloseFile(fd1));               //Κλείσιμο αρχείου και αποδέσμευση μνήμης
  VALUE_CALL_OR_DIE(BF_Close());
  ...
  VALUE_CALL_OR_DIE(BF_OpenFile("block_example.db", &fd1));
  int blocks_num;
  VALUE_CALL_OR_DIE(BF_GetBlockCounter(fd1, &blocks_num));

  for (int i = 0; i < blocks_num; ++i) {
    printf("Contents of Block %d\n\t",i);
    VALUE_CALL_OR_DIE(BF_GetBlock(fd1, i, block));
  ...
  VALUE_CALL_OR_DIE(BF_CloseFile(fd1));
  VALUE_CALL_OR_DIE(BF_Close());
